...
This specification defines the requirements for OCI SDK adapter implementations that bridge the application's ports layer to Oracle Cloud Infrastructure services.
...
The system SHALL provide an `OciAuthProviderFactory` for creating OCI SDK authentication providers.

#### Scenario: Create auth provider from config file

- **GIVEN** a valid OCI config file at the default path
- **WHEN** `create(ociConfig)` is called
- **THEN** the factory returns a valid `AuthenticationDetailsProvider`

#### Scenario: Handle missing config file

- **GIVEN** no OCI config file exists at the specified path
- **WHEN** `create()` is called
- **THEN** the factory throws an appropriate exception with helpful message
