...
- [x] 1.1 Create `api/` package under `integration/` for API-level tests
...
- [x] 2.1 Create `AlertApiIT.java` with test class extending `IntegrationTestBase`
...
- [x] 2.2 Implement test: POST `/api/v1/alerts` with valid JSON → 200 + ChecklistResponse
...
- [x] 2.3 Implement test: POST `/api/v1/alerts` with invalid severity → 400 + ErrorResponse
...
- [x] 2.4 Implement test: POST `/api/v1/alerts` with missing required fields → 400 + validation error
...
- [x] 2.5 Implement test: Verify JSON response structure matches DTOs
...
- [x] 3.1 Create `HealthApiIT.java` with test class extending `IntegrationTestBase`
  - **Verification**: `mvn test-compile` succeeds; class is annotated with `@ServerTest`
  - **Acceptance**: Test class compiles and Helidon injects `WebServer`

- [x] 3.2 Implement test: GET `/api/v1/health` → 200 + `{"status": "UP", "timestamp": ...}`
...
- [x] 4.1 Run `mvn verify -Pit` to confirm all API integration tests pass
  - **Verification**: Full integration test suite green
  - **Acceptance**: Zero test failures; build succeeds

- [x] 4.2 Verify tests use Helidon's HTTP client to hit real endpoints (no mocked handlers)
