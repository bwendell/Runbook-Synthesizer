...
/**
 * Severity levels for alerts from OCI Monitoring Alarms and other sources. Used to categorize alert
 * urgency for prioritization in checklist generation.
 */
public enum AlertSeverity {
  ...
  /**
   * Parses a severity string into an AlertSeverity enum value. Case-insensitive matching.
   *
   * @param severity the severity string to parse (e.g., "critical", "CRITICAL", "Critical")
   * @return the matching AlertSeverity enum value
   * @throws IllegalArgumentException if the severity string is null or does not match any known
   *     value
   */
