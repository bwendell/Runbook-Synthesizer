 ...
 * <p>
 * Provides methods to list and retrieve markdown runbook files from OCI Object
 * Storage buckets.
 */
public class OciObjectStorageClient {
	...
	public OciObjectStorageClient(ObjectStorageClient objectStorageClient) {
		this.objectStorageClient = Objects.requireNonNull(objectStorageClient, "objectStorageClient cannot be null");
	}
	...
	public CompletableFuture<List<String>> listRunbooks(String namespace, String bucketName) {
		return CompletableFuture.supplyAsync(() -> {
			ListObjectsRequest request = ListObjectsRequest.builder().namespaceName(namespace).bucketName(bucketName)
					.build();

			ListObjectsResponse response = objectStorageClient.listObjects(request);

			return response.getListObjects().getObjects().stream().map(ObjectSummary::getName)
					.filter(name -> name.endsWith(MARKDOWN_SUFFIX)).collect(Collectors.toList());
		});
	}
	 ...
	 *            the bucket name
	 * @param objectName
	 *            the object name (file path)
	 * @return a CompletableFuture containing the file content, or empty if not
	 *         found
	 */
	public CompletableFuture<Optional<String>> getRunbookContent(String namespace, String bucketName,
			String objectName) {
		return CompletableFuture.supplyAsync(() -> {
			try {
				GetObjectRequest request = GetObjectRequest.builder().namespaceName(namespace).bucketName(bucketName)
