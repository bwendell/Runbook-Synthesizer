 ...
 *
 * <p>This interface defines the contract for the RAG retrieval component. Implementations combine
 * vector similarity search with metadata filtering to find the most relevant runbook sections for a
 * given alert context.
 *
 * @see RetrievedChunk
 * @see EnrichedContext
...
public interface RunbookRetriever {
   ...
   *
   * <ol>
   *   <li>Build a query from the EnrichedContext (alert + metrics + logs)
   *   <li>Perform vector similarity search against the runbook store
   *   <li>Apply metadata filtering/boosting (tags, applicable shapes)
   *   <li>Return ranked results with similarity scores
   * </ol>
   *
