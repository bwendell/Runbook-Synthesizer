...
/**
 * Default implementation of {@link RunbookRetriever} that combines vector 
 * similarity search with metadata-based re-ranking.
 * <p>
 * This implementation boosts chunks based on tag overlap and shape matching 
 * to improve relevance for specific resource contexts.
 *
...
public class DefaultRunbookRetriever implements RunbookRetriever {
	 ...
	 */
	public DefaultRunbookRetriever(EmbeddingService embeddingService, VectorStoreRepository vectorStore) {
	...
	private double calculateTagBoost(RunbookChunk chunk, EnrichedContext context) {
		...
		long matchCount = chunk.tags().stream()
				.filter(tag -> context.alert().dimensions().containsKey(tag) 
	...
	private double calculateShapeBoost(RunbookChunk chunk, EnrichedContext context) {
		if (chunk.applicableShapes() == null || chunk.applicableShapes().isEmpty() 
				|| context.resource() == null || context.resource().shape() == null) {
			return 0.0;
		}

		String resourceShape = context.resource().shape();
		boolean matches = chunk.applicableShapes().stream()
				.anyMatch(pattern -> matchesShape(pattern, resourceShape));
