...
/**
 * Default implementation of {@link EmbeddingService} that delegates to {@link LlmProvider} for
 * embedding generation.
 *
 * <p>This service provides a simplified facade over the LlmProvider's embedding capabilities,
 * decoupling embedding concerns from text generation concerns.
 *
...
public class DefaultEmbeddingService implements EmbeddingService {
   ...
   *
   * @param llmProvider the LLM provider for embedding generation
   * @throws NullPointerException if llmProvider is null
   */
   ...
   * {@inheritDoc}
   *
   * @throws NullPointerException if texts is null
   */
  @Override
  ...
  private String formatContextQuery(com.oracle.runbook.domain.EnrichedContext context) {
    ...
    if (context.resource() != null) {
      sb.append("Resource: ")
          .append(context.resource().displayName())
          .append(" (Shape: ")
          .append(context.resource().shape())
          .append(")\n");
    }
    return sb.toString();
  }
