...
/**
 * Domain service that orchestrates the Retrieval Augmented Generation (RAG)
 * pipeline to produce context-aware troubleshooting checklists.
 * <p>
 * The pipeline follows these steps:
 * <ol>
 * <li>Enrich the alert with real-time infrastructure context</li>
 * <li>Retrieve relevant runbook chunks based on the enriched context</li>
 * <li>Generate a dynamic checklist using the LLM and retrieved chunks</li>
 * </ol>
 */
public class RagPipelineService {
	 ...
	 *
	 * @param enrichmentService
	 *            the service used to gather infrastructure context
	 * @param retriever
	 *            the service used to fetch relevant runbook snippets
	 * @param generator
	 *            the service used to synthesize the final checklist
	 */
	public RagPipelineService(ContextEnrichmentService enrichmentService, RunbookRetriever retriever,
			ChecklistGenerator generator) {
		this.enrichmentService = Objects.requireNonNull(enrichmentService, "enrichmentService cannot be null");
	...
	/**
	 * Processes an incoming alert through the RAG pipeline to generate a
	 * troubleshooting guide.
	 *
	 * @param alert
	 *            the alert to process
	 * @param topK
	 *            the number of runbook chunks to retrieve for context
	...
	public CompletableFuture<DynamicChecklist> processAlert(Alert alert, int topK) {
		...
		return enrichmentService.enrich(alert).thenApply(context -> {
