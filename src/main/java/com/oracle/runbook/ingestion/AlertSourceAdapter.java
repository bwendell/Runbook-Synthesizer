 ...
 * Hexagonal Architecture. Implementations transform source-specific
 * alert payloads (e.g., OCI Monitoring Alarms, OCI Events) into the
 * canonical {@link Alert} domain model.
 * <p>
 * Unlike async enrichment operations, parsing is synchronous since
 * it's fast and doesn't require external calls.
 *
 * @see Alert
 */
public interface AlertSourceAdapter {
     ...
     * <p>
     * Implementations should extract relevant fields from the source-specific
     * JSON format and map them to the Alert record.
     *
     ...
     * <p>
     * Used for routing incoming payloads to the correct adapter when
