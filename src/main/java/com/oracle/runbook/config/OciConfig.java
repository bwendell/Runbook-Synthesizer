 ...
 * <p>
 * Holds the necessary configuration for connecting to OCI services, including
 * the compartment ID, region, config file path, and profile name.
 *
 * @param compartmentId
 *            the OCI compartment OCID (required)
 * @param region
 *            the OCI region identifier (optional, e.g., "us-ashburn-1")
 * @param configFilePath
 *            the path to OCI config file (optional, defaults to ~/.oci/config)
 * @param profile
 *            the OCI config profile name (optional, defaults to "DEFAULT")
 */
public record OciConfig(String compartmentId, String region, String configFilePath, String profile) {
	...
	public static OciConfig withDefaults(String compartmentId) {
		return new OciConfig(compartmentId, null, "~/.oci/config", "DEFAULT");
	}
}
