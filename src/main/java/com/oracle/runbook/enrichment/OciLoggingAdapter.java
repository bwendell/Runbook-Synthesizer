 ...
 * <p>
 * Fetches logs from OCI Logging Search API and converts them to domain model
 * objects. Uses the OCI Java SDK LogSearchClient for API calls.
 */
public class OciLoggingAdapter implements LogSourceAdapter {
	 ...
	 *
	 * @param logSearchClient
	 *            the OCI Log Search client
	 * @param config
	 *            the OCI configuration
	 */
	public OciLoggingAdapter(LogSearchClient logSearchClient, OciConfig config) {
		this.logSearchClient = Objects.requireNonNull(logSearchClient, "logSearchClient cannot be null");
	...
	public CompletableFuture<List<LogEntry>> fetchLogs(String resourceId, Duration lookback, String query) {
		return CompletableFuture.supplyAsync(() -> {
			...
			SearchLogsDetails details = SearchLogsDetails.builder().searchQuery(searchQuery)
					.timeStart(Date.from(startTime)).timeEnd(Date.from(endTime)).isReturnFieldInfo(false).build();

			SearchLogsRequest request = SearchLogsRequest.builder().searchLogsDetails(details).limit(DEFAULT_LIMIT)
					.build();
	...
	private Map<String, String> extractMetadata(Map<String, Object> data) {
		...
		for (Map.Entry<String, Object> entry : data.entrySet()) {
			...
			// Skip known fields
			if (!key.equals("logId") && !key.equals("datetime") && !key.equals("level") && !key.equals("message")) {
