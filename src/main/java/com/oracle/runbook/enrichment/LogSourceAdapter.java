 ...
 * <p>
 * This interface defines the contract for log source adapters in the Hexagonal
 * Architecture. Implementations provide concrete integrations with specific log
 * backends like OCI Logging, Grafana Loki, etc.
 * <p>
...
public interface LogSourceAdapter {
	 ...
	 *
	 * @param resourceId
	 *            the identifier of the resource to fetch logs for (e.g., OCID for
	 *            OCI, label selector for Loki)
	 * @param lookback
	 *            the time duration to look back from now
	 * @param query
	 *            source-specific query string (may be null or empty)
	 * @return a CompletableFuture containing the list of log entries, never null
	 *         (may complete with empty list)
	 */
