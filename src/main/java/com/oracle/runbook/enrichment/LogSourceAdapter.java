 ...
 *
 * <p>This interface defines the contract for log source adapters in the Hexagonal Architecture.
 * Implementations provide concrete integrations with specific log backends like OCI Logging,
 * Grafana Loki, etc.
 *
...
public interface LogSourceAdapter {
   ...
   *
   * <p>The query parameter allows source-specific query syntax:
   *
   * <ul>
   *   <li>OCI Logging: filter expressions
   *   <li>Grafana Loki: LogQL queries
   * </ul>
   *
   * @param resourceId the identifier of the resource to fetch logs for (e.g., OCID for OCI, label
   *     selector for Loki)
   * @param lookback the time duration to look back from now
   * @param query source-specific query string (may be null or empty)
   * @return a CompletableFuture containing the list of log entries, never null (may complete with
   *     empty list)
   */
