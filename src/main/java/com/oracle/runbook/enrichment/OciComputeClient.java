...
public class OciComputeClient {
	 ...
	 * @param instanceOcid
	 *            the instance OCID
	 * @return a CompletableFuture containing the instance metadata, or empty if not
	 *         found
	 */
	public CompletableFuture<Optional<ResourceMetadata>> getInstance(String instanceOcid) {
		return CompletableFuture.supplyAsync(() -> {
			try {
				GetInstanceRequest request = GetInstanceRequest.builder().instanceId(instanceOcid).build();
	...
	private ResourceMetadata convertToResourceMetadata(Instance instance) {
		return new ResourceMetadata(instance.getId(), instance.getDisplayName(), instance.getCompartmentId(),
				instance.getShape(), instance.getAvailabilityDomain(),
	 ...
	 * <p>
	 * OCI defined tags are structured as Map<String, Map<String, Object>>. This
	 * method flattens them to Map<String, String> with "namespace.key" format.
	 */
