...
class ContextEnrichmentServiceTest {
	...
	private Alert createTestAlert() {
		return new Alert("alert-001", "High Memory Usage", "Memory utilization exceeded 90% threshold",
				AlertSeverity.WARNING, "oci-monitoring", Map.of("resourceId", "ocid1.instance.oc1..example"), Map.of(),
				Instant.now(), "{}");
	}

	/**
	 * Test implementation of ContextEnrichmentService for verifying interface
	 * contract.
	 */
	private static class TestContextEnrichmentService implements ContextEnrichmentService {
		...
		public CompletableFuture<EnrichedContext> enrich(Alert alert) {
			ResourceMetadata resource = new ResourceMetadata(alert.dimensions().get("resourceId"), "test-server",
					"ocid1.compartment.oc1..example", "VM.Standard2.1", "AD-1", Map.of(), Map.of());

			MetricSnapshot metric = new MetricSnapshot("CpuUtilization", "oci_computeagent", 45.5, "percent",
					Instant.now());

			EnrichedContext context = new EnrichedContext(alert, resource, List.of(metric), List.of(), Map.of());
