 ...
 * <p>
 * Note: Full integration testing with OCI SDK requires live credentials. These
 * unit tests verify the adapter implements the interface contract.
 */
class OciMonitoringAdapterTest {
	...
	void testConstructorRejectsNullConfig() {
		...
		assertThrows(NullPointerException.class, () -> new OciMonitoringAdapter(null, null));
	}
	...
	void testFetchMetricsContract() {
		...
		try {
			var method = OciMonitoringAdapter.class.getMethod("fetchMetrics", String.class, java.time.Duration.class);
