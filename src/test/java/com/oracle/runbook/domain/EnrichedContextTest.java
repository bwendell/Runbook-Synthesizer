...
class EnrichedContextTest {
    ...
    void constructionWithAllComponentsSucceeds() {
        ...
        List<MetricSnapshot> metrics = List.of(
            new MetricSnapshot("CpuUtilization", "oci", 85.0, "%", Instant.now())
        );
        List<LogEntry> logs = List.of(
            new LogEntry("log-1", Instant.now(), "ERROR", "error msg", Map.of())
        );
    ...
    void throwsForNullAlert() {
        assertThrows(NullPointerException.class, () -> new EnrichedContext(
            null,
            createTestMetadata(),
            List.of(),
            List.of(),
    ...
    void metricsListIsImmutable() {
        ...
        assertThrows(UnsupportedOperationException.class, 
            () -> context.recentMetrics().add(null));
    }
