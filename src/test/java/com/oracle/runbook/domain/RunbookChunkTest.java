...
class RunbookChunkTest {
	...
	void constructionWithAllFieldsSucceeds() {
		...
		RunbookChunk chunk = new RunbookChunk("chunk-123", "runbooks/memory/high-memory.md", "Step 3: Check for OOM",
				"Run dmesg to check for OOM killer events...", tags, shapes, embedding);
	...
	void embeddingArrayIsDefensivelyCopied() {
		...
		RunbookChunk chunk = new RunbookChunk("id", "path", "title", "content", List.of(), List.of(),
				originalEmbedding);
	...
	void tagsListIsImmutable() {
		...
		RunbookChunk chunk = new RunbookChunk("id", "path", "title", "content", mutableTags, List.of(), new float[0]);
	...
	void applicableShapesListIsImmutable() {
		...
		RunbookChunk chunk = new RunbookChunk("id", "path", "title", "content", List.of(), mutableShapes, new float[0]);
