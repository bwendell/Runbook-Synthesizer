...
class DynamicChecklistTest {
	...
	private ChecklistStep createTestStep(int order) {
		return new ChecklistStep(order, "Step " + order + " instruction", "Rationale for step " + order, "current",
				"expected", StepPriority.MEDIUM, List.of("command-" + order));
	}
	...
	void constructionWithAllFieldsSucceeds() {
		...
		DynamicChecklist checklist = new DynamicChecklist("alert-123",
				"This checklist addresses high memory usage on web-server-01", steps, sourceRunbooks, now, "oci-genai");
	...
	void stepsListIsImmutable() {
		...
		DynamicChecklist checklist = new DynamicChecklist("alert-123", "summary", mutableSteps, List.of(),
	...
	void sourceRunbooksListIsImmutable() {
		...
		DynamicChecklist checklist = new DynamicChecklist("alert-123", "summary", List.of(), mutableRunbooks,
				Instant.now(), "provider");
		...
		// Checklist's list should be unmodifiable
		assertThrows(UnsupportedOperationException.class, () -> checklist.sourceRunbooks().add("another"));
	}
}
