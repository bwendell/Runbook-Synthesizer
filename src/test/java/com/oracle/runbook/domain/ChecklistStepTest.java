...
class ChecklistStepTest {
    ...
    void throwsForNullInstruction() {
        assertThrows(NullPointerException.class, () -> new ChecklistStep(
            1, null, "rationale", "current", "expected",
            StepPriority.MEDIUM, List.of()
        ));
    }
    ...
    void commandsListIsImmutable() {
        ...
        ChecklistStep step = new ChecklistStep(
            1, "instruction", "rationale", "current", "expected",
            StepPriority.LOW, mutableCommands
        );
        ...
        assertThrows(UnsupportedOperationException.class, 
            () -> step.commands().add("newCommand"));
    }

    @Test
    @DisplayName("ChecklistStep accepts zero order")
    void acceptsZeroOrder() {
        assertDoesNotThrow(() -> new ChecklistStep(
            0, "instruction", null, null, null,
            StepPriority.MEDIUM, List.of()
        ));
    }
}
