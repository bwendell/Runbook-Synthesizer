...
class ChecklistStepTest {
	...
	void constructionWithAllFieldsSucceeds() {
		...
		ChecklistStep step = new ChecklistStep(1, "Check current memory usage",
				"Memory issues often manifest as high utilization", "92%", "<80%", StepPriority.HIGH, commands);
	...
	void throwsForNullInstruction() {
		assertThrows(NullPointerException.class,
				() -> new ChecklistStep(1, null, "rationale", "current", "expected", StepPriority.MEDIUM, List.of()));
	}
	...
	void commandsListIsImmutable() {
		...
		ChecklistStep step = new ChecklistStep(1, "instruction", "rationale", "current", "expected", StepPriority.LOW,
				mutableCommands);
	...
	void acceptsZeroOrder() {
		assertDoesNotThrow(() -> new ChecklistStep(0, "instruction", null, null, null, StepPriority.MEDIUM, List.of()));
	}
}
