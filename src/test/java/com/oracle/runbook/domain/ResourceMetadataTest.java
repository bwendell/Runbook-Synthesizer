...
class ResourceMetadataTest {
	...
	void constructionWithAllFieldsSucceeds() {
		...
		ResourceMetadata metadata = new ResourceMetadata("ocid1.instance.oc1.iad.xxx", "web-server-01",
				"ocid1.compartment.oc1..xxx", "VM.Standard2.4", "AD-1", freeformTags, definedTags);
	...
	void freeformTagsMapIsImmutable() {
		...
		ResourceMetadata metadata = new ResourceMetadata("ocid1.instance.oc1.iad.xxx", "name", "compartment", "shape",
				"ad", mutableTags, Map.of());
		...
		assertThrows(UnsupportedOperationException.class, () -> metadata.freeformTags().put("another", "value"));
	}
	...
	void definedTagsMapIsImmutable() {
		...
		// Record's map should be unmodifiable
		assertThrows(UnsupportedOperationException.class, () -> metadata.definedTags().put("another", "value"));
	}
}
