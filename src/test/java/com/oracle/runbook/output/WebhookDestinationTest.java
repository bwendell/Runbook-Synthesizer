...
/**
 * Tests for the WebhookDestination interface and WebhookResult record
 * contracts.
 */
class WebhookDestinationTest {
	...
	void webhookResult_capturesResponseDetails() {
		...
		WebhookResult failResult = new WebhookResult(false, 500, "", Optional.of("Internal server error"));
	...
	private DynamicChecklist createTestChecklist() {
		ChecklistStep step = new ChecklistStep(1, "Check memory usage", "Memory is at 92%", "92%", "Below 80%",
				StepPriority.HIGH, List.of("free -h"));
		return new DynamicChecklist("alert-001", "High memory troubleshooting", List.of(step),
