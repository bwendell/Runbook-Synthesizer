...
class RagPipelineServiceTest {
	...
	private Alert createTestAlert() {
		return new Alert("a1", "Title", "Msg", AlertSeverity.CRITICAL, "oci", 
	...
	private static class StubEnrichmentService implements ContextEnrichmentService {
		...
		void setResult(EnrichedContext r) { this.result = r; }
		@Override public CompletableFuture<EnrichedContext> enrich(Alert alert) {
			this.called = true;
	...
	private static class StubRetriever implements RunbookRetriever {
		...
		void setResult(List<RetrievedChunk> r) { this.result = r; }
		@Override public List<RetrievedChunk> retrieve(EnrichedContext context, int topK) {
			this.called = true;
			return result;
		}
	}
	...
	private static class StubGenerator implements ChecklistGenerator {
		...
		void setResult(DynamicChecklist r) { this.result = r; }
		@Override public DynamicChecklist generate(EnrichedContext context, List<RetrievedChunk> chunks) {
			this.called = true;
			return result;
		}
	}
