...
class OciObjectStorageClientTest {
	...
	void testListRunbooksContract() {
		try {
			var method = OciObjectStorageClient.class.getMethod("listRunbooks", String.class, String.class);
	...
	void testGetRunbookContentContract() {
		try {
			var method = OciObjectStorageClient.class.getMethod("getRunbookContent", String.class, String.class,
					String.class);
			assertTrue(method.getReturnType().equals(CompletableFuture.class));
		} catch (NoSuchMethodException e) {
