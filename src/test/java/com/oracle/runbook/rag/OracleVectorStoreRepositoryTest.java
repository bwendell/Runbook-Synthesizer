...
class OracleVectorStoreRepositoryTest {
	...
	void store_convertsChunkToSegmentAndStores() {
		// Arrange
		RunbookChunk chunk = createTestChunk("chunk-001", "Check memory usage", 
				List.of("memory", "linux"), List.of("VM.*"));
		...
		assertEquals("runbooks/memory/high-memory.md", 
				stubEmbeddingStore.lastStoredTextSegment.metadata().getString("runbookPath"));
		assertEquals("memory,linux", 
				stubEmbeddingStore.lastStoredTextSegment.metadata().getString("tags"));
	}
	...
	private RunbookChunk createTestChunk(String id, String content, List<String> tags, 
			List<String> applicableShapes) {
		return new RunbookChunk(id, "runbooks/memory/high-memory.md", "Section Title",
				content, tags, applicableShapes, new float[] { 0.1f, 0.2f, 0.3f });
	}
