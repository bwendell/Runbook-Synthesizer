...
class DefaultChecklistGeneratorTest {
	...
	void generate_formatsPromptAndParsesChecklist() {
		...
		assertTrue(llmProvider.lastPrompt.contains("Restart the service")); // Verify chunk was in prompt
	}
	...
	private EnrichedContext createTestContext() {
		Alert alert = new Alert("a1", "High CPU", "CPU spike", AlertSeverity.CRITICAL, "oci",
				Map.of(), Map.of(), Instant.now(), "{}");
		ResourceMetadata resource = new ResourceMetadata("r1", "web01", "comp1", "VM.Standard2.1", "AD1", Map.of(), Map.of());
	...
	private RetrievedChunk createRetrievedChunk(String id, String content) {
		RunbookChunk chunk = new RunbookChunk(id, "path", "Title", content, List.of(), List.of(), new float[0]);
	...
	private static class StubLlmProvider implements LlmProvider {
		...
		void setNextResponse(String r) { this.nextResponse = r; }
		...
		@Override public CompletableFuture<float[]> generateEmbedding(String text) { return null; }
		@Override public CompletableFuture<List<float[]>> generateEmbeddings(List<String> texts) { return null; }
	}
}
