...
class ChecklistGeneratorTest {
	...
	private EnrichedContext createTestContext() {
		Alert alert = new Alert("alert-001", "High Memory", "Memory utilization exceeded threshold",
				AlertSeverity.WARNING, "oci-monitoring", Map.of("resourceId", "ocid1.instance.oc1..example"), Map.of(),
				Instant.now(), "{}");
	...
	private List<RetrievedChunk> createTestChunks() {
		...
		RunbookChunk chunk = new RunbookChunk("chunk-001", "runbooks/memory/high-memory.md", "Step 1: Check memory",
				"Run free -h to check memory usage", List.of("memory"), List.of("VM.*"), embedding);
	...
	private static class TestChecklistGenerator implements ChecklistGenerator {
		...
		public DynamicChecklist generate(EnrichedContext context, List<RetrievedChunk> relevantChunks) {
			ChecklistStep step = new ChecklistStep(1, "Check memory usage with free -h", "Memory is at 92%", "92%",
					"Below 80%", StepPriority.HIGH, List.of("free -h"));
			return new DynamicChecklist(context.alert().id(), "High memory troubleshooting checklist", List.of(step),
					List.of("runbooks/memory/high-memory.md"), Instant.now(), "oci-genai");
		}
	}
}
