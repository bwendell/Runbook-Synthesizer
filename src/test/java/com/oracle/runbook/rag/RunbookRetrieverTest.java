...
class RunbookRetrieverTest {
	...
	private static class TestRunbookRetriever implements RunbookRetriever {
		...
		public List<RetrievedChunk> retrieve(EnrichedContext context, int topK) {
			...
			RunbookChunk chunk1 = new RunbookChunk("chunk-001", "runbooks/memory/high-memory.md",
					"Step 1: Check memory", "Run free -h to check memory usage", List.of("memory", "linux"),
					List.of("VM.*"), embedding);
			RunbookChunk chunk2 = new RunbookChunk("chunk-002", "runbooks/memory/high-memory.md",
					"Step 2: Find memory consumers", "Run ps aux --sort=-%mem | head -20", List.of("memory", "process"),
					List.of("VM.*"), embedding);
			return List.of(new RetrievedChunk(chunk1, 0.92, 0.1, 1.02), new RetrievedChunk(chunk2, 0.88, 0.05, 0.93));
		}
	}
}
